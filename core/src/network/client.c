...
void clientReceiveUpdateEntity(TCP_CLIENT* client, U8* buffer) {
    ...
    VECTORI previousChunkPosition = client->chunkPosition;
    ...
    if (px != previousChunkPosition.x || py != previousChunkPosition.y || pz != previousChunkPosition.z)
        worldUpdateClientChunk(client, previousChunkPosition);
